...
public:
    BufferNotFoundException(quint16 id) throw() :
        BufferException(ProtocolError::WRONG_BUFFER_ID, QObject::tr("Buffer with id '%1' was not found").arg(id))
    {}
};
...
public:
    WrongPushedDataSizeException(quint16 arraySize, quint16 bufferSize) throw() :
        BufferException(ProtocolError::WRONG_INPUT_ARRAY_SIZE, QObject::tr("Pushing array's size(%1) is not equal buffer's size(%2)").arg(arraySize).arg(bufferSize))
    {}
};
...
public:
    WrongRequestTypeException(quint8 type) :
        ProtocolException(ProtocolError::WRONG_REQUEST_TYPE, QObject::tr("Wrong request type: %1").arg(type))
    {}
};
...
public:
    WrongTimeStampException(TimeStamp timeStamp) :
        BufferException(ProtocolError::WRONG_TIME_STAMP, QObject::tr("Timestamp '%1' does not exists in buffers").arg(timeStamp))
    {}
};
}
